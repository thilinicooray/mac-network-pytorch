...
class BaseModel(nn.Module):
    ...
    def forward_eval(self, image, topk=5):
        ...
        verb_pred_sm = F.softmax(verb_pred, dim=-1)
        verb_pred_sm = verb_pred_sm.contiguous().view(batch_size, -1, self.n_verbs)

        verb_wise_tot = torch.sum(verb_pred_sm, 1)

        return verb_wise_tot
