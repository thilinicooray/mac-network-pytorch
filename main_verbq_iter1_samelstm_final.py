...
import json
import model_verbq_iter1_samelstm_final
import os
import utils
import time
import random
...
def train(model, train_loader, dev_loader, traindev_loader, optimizer, scheduler, max_epoch, model_dir, encoder, gpu_mode, clip_norm, lr_max, model_name, args,eval_frequency=4000):
    ...
    for epoch in range(max_epoch):
        ...
        for i, (id, img, verb, labels) in enumerate(train_loader):
            ...
            '''print('grad check verb:')
            for f in model.verb_module.verb_vqa.parameters():
                print('data is')
                print(f.data[0][0])
                #print('grad is')
                #print(f.grad)
            ...
            print('grad check role:')
            for f in model.role_module.lstm_proj2.parameters():
                print('data is')
                print(f.data[0][0])
                #print('grad is')
                #print(f.grad)
            ...
            if total_steps % eval_frequency == 0:
                ...
                if max_score == dev_score_list[-1]:
                    torch.save(model.state_dict(), model_dir + "/{}_verbq_iter1_samelstm_final.model".format( model_name))
...
def main():
    ...
    model = model_verbq_iter1_samelstm_final.BaseModel(encoder, args.gpuid)
