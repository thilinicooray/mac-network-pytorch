...
class WriteUnit(nn.Module):
    def __init__(self, dim, max_role, self_attention=False, memory_gate=False):
        ...
        self.role_dep = linear(dim * 2, dim)
    ...
    def forward(self, memories, retrieved, controls, mask):
        ...
        retrieved = retrieved.unsqueeze(1).expand(batch_size, self.max_role, self.max_role, self.dim)
        retrieved = retrieved.contiguous().view(-1, self.max_role, self.dim)

        concat_vec = torch.cat([retrieved_ind, retrieved], 2).view(-1, self.n_concat)
        #print('no issue after cat')
        rolewise_msg = torch.tanh(self.role_dep(concat_vec))

        rolewise_msg = rolewise_msg.view(-1, self.max_role, self.max_role, self.dim)
        #print('write :', mask.size(), rolewise_msg.size())
        msg_with_neighbours = (mask * rolewise_msg).view(-1, self.max_role, self.dim).sum(1).squeeze()

        next_mem = self.rnn(msg_with_neighbours, prev_mem)

        return next_mem


class MACUnit(nn.Module):
    def __init__(self, dim, max_role, max_step=12,
        ...
        self.write = WriteUnit(dim, self.max_role, self_attention, memory_gate)
