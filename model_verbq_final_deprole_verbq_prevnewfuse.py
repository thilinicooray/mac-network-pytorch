...
class TopDown(nn.Module):
    ...
    def forward(self, img, q, iter0_rep):
        ...
        v_emb_with_q = iter0_rep + self.dropout(v_emb_with_q)
...
class BaseModel(nn.Module):
    ...
    def forward(self, img, verb, labels):
        ...
        lstm_out, (h, _) = self.verb_module.verb_vqa.q_emb(q_emb)
        q_emb = h.permute(1, 0, 2).contiguous().view(batch_size, -1)
        ...
        if self.training:
            ...
            v_emb_with_q = v_emb_with_q.expand(3,v_emb_with_q.size(0), v_emb_with_q.size(1))
            v_emb_with_q = v_emb_with_q.transpose(0,1)
            v_emb_with_q = v_emb_with_q.contiguous().view(batch_size* 3, -1)

            verb_pred = self.verb_vqa(img_embd, q_emb, v_emb_with_q)
            verb_pred = self.last_class(verb_pred)
            verb_pred = verb_pred.contiguous().view(batch_size, -1, self.n_verbs)


        else:
            verb_pred_prev = self.verb_module.last_class(self.verb_module.verb_vqa.classifier(v_emb_with_q))
            ...
            verb_pred_logit = self.verb_vqa(img_embd, q_emb, v_emb_with_q)
