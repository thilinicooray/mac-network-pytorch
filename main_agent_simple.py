...
from imsitu_loader import imsitu_loader_agent
from imsitu_scorer_log import imsitu_scorer
import json
import model_agent_simple
import os
import utils
        ...
        for i, (id, img, labels) in enumerate(train_loader):
            ...
            if total_steps % eval_frequency == 0:
                ...
                if max_score == dev_score_list[-1]:
                    torch.save(model.state_dict(), model_dir + "/{}_agentall_withnone.model".format(model_name))
...
def main():
    ...
    model = model_agent_simple.BaseModel(encoder, args.gpuid)
    ...
    train_set = imsitu_loader_agent(imgset_folder, train_set, encoder, model.train_preprocess())

    train_loader = torch.utils.data.DataLoader(train_set, batch_size=64, shuffle=True, num_workers=n_worker)

    dev_set = json.load(open(dataset_folder +"/dev.json"))
    dev_set = imsitu_loader_agent(imgset_folder, dev_set, encoder, model.dev_preprocess())
    ...
    test_set = imsitu_loader_agent(imgset_folder, test_set, encoder, model.dev_preprocess())
    ...
    traindev_set = imsitu_loader_agent(imgset_folder, traindev_set, encoder, model.dev_preprocess())
