...
def train(model, train_loader, dev_loader, traindev_loader, optimizer, scheduler, max_epoch, model_dir, encoder, gpu_mode, clip_norm, lr_max, model_name, args,eval_frequency=4000):
    ...
    for epoch in range(max_epoch):
        ...
        mx = len(train_loader)
        for i, (_, img, verb, roles,role_q, q_len, labels) in enumerate(train_loader):
            ...
            noun_predict, role_predict = pmodel(img, verb)
            ...
            top1.add_point_noun(verb, noun_predict, labels)
            top5.add_point_noun(verb, noun_predict, labels)
            ...
            del noun_predict, role_predict, loss, img, verb, roles, labels
            #break
...
def eval(model, dev_loader, encoder, gpu_mode, write_to_file = False):
    ...
    with torch.no_grad():
        ...
        for i, (img_id, img, verb, roles,role_q, q_len, labels) in enumerate(dev_loader):
            ...
            noun_predict, role_predict = model(img, verb)
            '''loss = model.calculate_eval_loss(verb_predict, verb, role_predict, labels)
            val_loss += loss.item()'''
            if write_to_file:
                top1.add_point_noun_log(img_id, verb, noun_predict, labels)
                top5.add_point_noun_log(img_id, verb, noun_predict, labels)
            else:
                top1.add_point_noun(verb, noun_predict, labels)
                top5.add_point_noun(verb, noun_predict, labels)

            del noun_predict, role_predict, img, verb, roles, labels
            #break
