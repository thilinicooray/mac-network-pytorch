import torch
from imsitu_encoder_roleq_verbtemplate import imsitu_encoder
from imsitu_loader import imsitu_loader_hico
from imsitu_scorer_log import imsitu_scorer
import json
import model_vsrl4hico
import os
import utils
import time
import random
    ...
    for epoch in range(max_epoch):
        ...
        for i, (id, img, verb) in enumerate(train_loader):
            ...
            if total_steps % print_freq == 0:
                top1_a = top1.get_average_results_hico()
                print ("{},{},{}, map = {:.2f}, loss = {:.2f}, avg loss = {:.2f}"
                       .format(total_steps-1,epoch,i, top1_a*100, loss.item(),
                               train_loss / ((total_steps-1)%eval_frequency) ))


            if total_steps % eval_frequency == 0:
                top1 = eval(model, dev_loader, encoder, gpu_mode)
                model.train()

                top1_avg = top1.get_average_results_hico()

                avg_score = top1_avg

                print ('Dev {} average :{:.2f} map :{:.2f}'.format(total_steps-1, avg_score*100,
                                                             top1_avg*100))
                ...
                if max_score == dev_score_list[-1]:
                    torch.save(model.state_dict(), model_dir + "/{}_vsrl4_hico.model".format( model_name))
...
def main():
    ...
    parser.add_argument('--vsrl_model', type=str, default='', help='pretrained verb module')
    ...
    model = model_vsrl4hico.BaseModel(encoder, args.gpuid)
    ...
    hico_train_set = json.load(open(dataset_folder +"/hico_train.json"))
    ...
    traindev_set = json.load(open(dataset_folder +"/hico_test.json"))
    traindev_set = imsitu_loader_hico(imgset_folder, traindev_set, encoder, model.dev_preprocess())
    traindev_loader = torch.utils.data.DataLoader(traindev_set, batch_size=8, shuffle=True, num_workers=n_worker)


    utils.load_net(args.vsrl_model, [model])
    model_name = 'train_hico'
