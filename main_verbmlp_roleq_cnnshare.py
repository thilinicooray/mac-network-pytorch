...
import json
import model_verbmlp_roleq_cnnshare
import os
import utils
import time
import random
...
def train(model, train_loader, dev_loader, traindev_loader, optimizer, scheduler, max_epoch, model_dir, encoder, gpu_mode, clip_norm, lr_max, model_name, args,eval_frequency=4000):
    ...
    for epoch in range(max_epoch):
        ...
        for i, (img_id, img, verb, roles,role_q, q_len, labels) in enumerate(train_loader):
            ...
            verb_predict, label_predict = pmodel(img, verb)
            ...
            loss = model.calculate_loss(verb_predict, verb, label_predict, labels, args)
            ...
            top1.add_point(verb_predict, verb, label_predict, labels)
            top5.add_point(verb_predict, verb, label_predict, labels)
            ...
            if total_steps % eval_frequency == 0:
                ...
                if max_score == dev_score_list[-1]:
                    torch.save(model.state_dict(), model_dir + "/{}_verbmlp_roleq_cnnshare.model".format( model_name))
            ...
            del verb_predict, label_predict, loss, img, verb, labels
            #break
...
def main():
    ...
    model = model_verbmlp_roleq_cnnshare.BaseModel(encoder, args.gpuid)
