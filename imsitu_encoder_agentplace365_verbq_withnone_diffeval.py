...
class imsitu_encoder():
    def __init__(self, train_set, verb_questions, nouns, places):
    ...
    def get_commonq_idx(self, q_words):
        commonq = 'what is the UNK doing at the UNK'
    ...
    def get_qword_idx_for_agentq_top1(self, agent_set, img_ids):
        agent_questions = []
        common_q = self.common_q_idx
        for i in range(len(agent_set)):
            full_q_idx = []
            idx = self.label_2_qword[agent_set[i]]

            if idx == -1:
                q = 'what is the action happening at the ' + self.places[img_ids[i]]
            else:
                full_q_idx = common_q[:3].tolist()
                full_q_idx.append(idx)
                full_q_idx.extend(common_q[4:-1].tolist())
                full_q_idx.append(self.question_words[self.places[img_ids[i]]])
            ...
            if len(full_q_idx) == 0:
                ...
                for word in words:
                    if word in self.question_words:
                        full_q_idx.append(self.question_words[word])
                    else:
                        full_q_idx.append(len(self.question_words))
                padding_words = self.max_q_word_count - len(full_q_idx)

                for w in range(padding_words):
                    full_q_idx.append(len(self.question_words))

            '''print(full_q_idx)
            for idx in full_q_idx:
                for word in self.question_words:
                    if self.question_words[word] == idx:
                        print(word)'''
            agent_questions.append(torch.tensor(full_q_idx))

        return torch.stack(agent_questions,0)
    ...
    def get_places_batch(self, img_id):
        places = []

        for id in img_id:
            places.append(self.question_words[self.places[id]])

        return torch.tensor(places)
