...
class Question_emb(nn.Module):
    def __init__(self,
                 embed_hidden=300,
                 mlp_hidden=512):
        super(Question_emb, self).__init__()
...
class BaseModel(nn.Module):
    ...
    def forward(self, image, verbq):
        ...
        img_feat = self.verb(self.conv_verb(image))

        q_emb = self.question_emb(verb_q)

        q_repr = self.q_net(q_emb)
        v_repr = self.v_net(img_feat)
