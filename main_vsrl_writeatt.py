        ...
        for i, (_, img, verb, roles,labels) in enumerate(train_loader):
            ...
            if total_steps % eval_frequency == 0:
                ...
                if max_score == dev_score_list[-1]:
                    torch.save(model.state_dict(), model_dir + "/{}_macnet4layer_gtv_writeatt.model".format( model_name))
...
def main():
    ...
    print('model spec :, mac net v att for write mac, lr decay at 10 by 0.85 ')

    train_set = json.load(open(dataset_folder + "/train.json"))
    encoder = imsitu_encoder(train_set)

    model = model_mac_writeatt.E2ENetwork(encoder, args.gpuid)
