...
class BaseModel(nn.Module):
    ...
    def forward(self, img, verb, labels):
        ...
        if self.training:
            verb_q_idx = self.encoder.get_verbq_idx(verb, labels[:,0,:]).unsqueeze(1)
            ...
            q_emb = self.verb_q_emb(verb_q_idx.squeeze())

            verb_pred = self.verb_vqa(img_embd, q_emb)

            verb_pred = verb_pred.contiguous().view(batch_size, self.n_verbs)
