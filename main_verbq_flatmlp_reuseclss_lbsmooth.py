...
import json
import model_verbq_flatmlp_reuseclss_lbsmooth
import os
import utils
import time
import random
...
def train(model, train_loader, dev_loader, traindev_loader, optimizer, scheduler, max_epoch, model_dir, encoder, gpu_mode, clip_norm, lr_max, model_name, args,eval_frequency=4000):
    ...
    for epoch in range(max_epoch):
        ...
        for i, (id, img, verb, labels) in enumerate(train_loader):
            ...
            loss = model.loss_criterion(verb_predict, verb)
            ...
            if total_steps % eval_frequency == 0:
                ...
                if max_score == dev_score_list[-1]:
                    torch.save(model.state_dict(), model_dir + "/{}_verbq_flatmlp_reuseclss_lbsmooth.model".format( model_name))
...
def main():
    ...
    model = model_verbq_flatmlp_reuseclss_lbsmooth.BaseModel(encoder, args.gpuid)
