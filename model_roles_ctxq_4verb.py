...
class TopDown(nn.Module):
    ...
    def forward(self, img_org, q):
        ...
        for i in range(3):
            ...
            att = self.v_att(img_org, q_emb_up)
            v_att_embd = (att * img_org)
            v_emb = v_att_embd.sum(1)
...
class BaseModel(nn.Module):
    ...
    def forward(self, img, verb):
        ...
        logits, v_att_embd = self.roles(img, role_verb)

        role_label_pred = logits.contiguous().view(batch_size, -1, self.vocab_size)
        v_att_embd = v_att_embd.contiguous().view(batch_size, self.max_role_count, -1, self.mlp_hidden)
        return role_label_pred, v_att_embd
