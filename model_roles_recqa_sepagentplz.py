...
class BaseModel(nn.Module):
    ...
    def forward(self, img_id, img, verb):
        ...
        place_role_label_pred = self.place_classifier(rep[:,0])
        agent_role_label_pred = self.agent_classifier(rep[:,1])
        other_role_label_pred = self.other_classifier(rep[:,2:])

        return place_role_label_pred, agent_role_label_pred, other_role_label_pred

    def calculate_loss(self, gt_verbs, place_role_label_pred, agent_role_label_pred, other_role_label_pred, gt_labels,args):

        batch_size = place_role_label_pred.size()[0]
        ...
        for i in range(batch_size):
            for index in range(gt_labels.size()[1]):
                ...
                for j in range(0, self.max_role_count):
                    if j==0:
                        #print(self.encoder.verb_list[gt_verbs[i]], gt_labels[i,index,j], self.vocab_size)
                        frame_loss += utils.cross_entropy_loss(place_role_label_pred[i], gt_labels[i,index,j] ,self.vocab_size)
                    elif j==1:
                        frame_loss += utils.cross_entropy_loss(agent_role_label_pred[i], gt_labels[i,index,j] ,self.vocab_size)
                    else:
                        frame_loss += utils.cross_entropy_loss(other_role_label_pred[i][j-2], gt_labels[i,index,j] ,self.vocab_size)
