        ...
        for i, (img, verb, roles,labels) in enumerate(train_loader):
            ...
            if total_steps % eval_frequency == 0:
                ...
                if max_score == dev_score_list[-1]:
                    torch.save(model.state_dict(), model_dir + "/{}_macnet4layer_gtv_2vgg.model".format( model_name))
...
def main():
    ...
    print('model spec :, mac net v pred for training and sep netw for verb and noun pred ')
    ...
    optimizer = torch.optim.Adam([{'params': model.conv.parameters(), 'lr': 5e-5},
                                  {'params': model.conv_nouns.parameters(), 'lr': 5e-5},
