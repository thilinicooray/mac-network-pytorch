...
from imsitu_loader import imsitu_loader, shuffle_minibatch
from imsitu_scorer_log import imsitu_scorer
import json
import mac_model_with_roleread
import os
import utils
import time
import random
        ...
        for i, (_, img, verb, roles,labels) in enumerate(train_loader):
            ...
            if total_steps % eval_frequency == 0:
                ...
                if max_score == dev_score_list[-1]:
                    torch.save(model.state_dict(), model_dir + "/{}_mac_roleread_optupdated_sampling4.model".format( model_name))
...
def main():
    ...
    print('model spec :, mac 4 layer roleread with sampler')

    train_set = json.load(open(dataset_folder + "/train.json"))
    encoder = imsitu_encoder(train_set)

    model = mac_model_with_roleread.E2ENetwork(encoder, args.gpuid)
    ...
    sampler = data_sampler.ImsituSampler(train_set, encoder.verb_list, encoder.verb_wise_items, samples_per_verb=4)
