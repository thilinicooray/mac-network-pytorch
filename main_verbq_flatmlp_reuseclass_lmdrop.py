import torch
from imsitu_encoder_verbq_with4roles import imsitu_encoder
from imsitu_loader import imsitu_loader_roleq_updated
from imsitu_scorer_log import imsitu_scorer
import json
import model_verbq_flatmlp_reuseclass_lmdrop
import os
import utils
import time
import random
        ...
        for i, (id, img, verb, labels) in enumerate(train_loader):
            ...
            if total_steps % eval_frequency == 0:
                ...
                if max_score == dev_score_list[-1]:
                    torch.save(model.state_dict(), model_dir + "/{}_verbq_flatmlp_reuseclss_4roles_lstmdrop.model".format( model_name))
...
def main():
    ...
    verb_templates = json.load(open("imsitu_data/verb_questions_4roletemplate.json"))
    encoder = imsitu_encoder(train_set, imsitu_roleq, verb_templates)

    model = model_verbq_flatmlp_reuseclass_lmdrop.BaseModel(encoder, args.gpuid)
