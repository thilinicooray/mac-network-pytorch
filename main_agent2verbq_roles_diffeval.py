import torch
from imsitu_encoder_agentverbq_roles_diffeval import imsitu_encoder
from imsitu_loader import imsitu_loader_agentverbq_roles
from imsitu_scorer_log import imsitu_scorer
import json
import model_agent2verbq_roles_diffeval
import os
import utils
...
def train(model, train_loader, dev_loader, traindev_loader, optimizer, scheduler, max_epoch, model_dir, encoder, gpu_mode, clip_norm, lr_max, model_name, args,eval_frequency=4000):
    ...
    for epoch in range(max_epoch):
        ...
        mx = len(train_loader)
        for i, (_id, img, verb, labels) in enumerate(train_loader):
            ...
            verb_predict, noun_pred = pmodel(img,_id, verb)

            '''g = make_dot(verb_predict, model.state_dict())
            g.view()'''

            loss = model.calculate_loss(verb_predict, verb, noun_pred, labels)
            ...
            if total_steps % eval_frequency == 0:
                ...
                if max_score == dev_score_list[-1]:
                    torch.save(model.state_dict(), model_dir + "/{}_agent2verbq_diffeval_beam_role.model".format(model_name))
...
def eval(model, dev_loader, encoder, gpu_mode, write_to_file = False):
    ...
    with torch.no_grad():
        mx = len(dev_loader)
        for i, (_id, img, verb, labels) in enumerate(dev_loader):
            ...
            val_loss += loss.item()'''
            top1.add_point_multi_verb_beam(_id, verb_predict, verb)
            top5.add_point_multi_verb_beam(_id, verb_predict, verb)
...
def main():
    ...
    imsitu_verbq = json.load(open("imsitu_data/verb_questions_new_with_unk.json"))
    space = json.load(open("imsitu_data/imsitu_space.json"))
    role_q = json.load(open("imsitu_data/imsitu_questions.json"))

    encoder = imsitu_encoder(train_set, imsitu_verbq, space['nouns'], role_q)

    model = model_agent2verbq_roles_diffeval.BaseModel(encoder, args.gpuid)
    ...
    train_set = imsitu_loader_agentverbq_roles(imgset_folder, train_set, encoder, model.train_preprocess())

    train_loader = torch.utils.data.DataLoader(train_set, batch_size=64, shuffle=True, num_workers=n_worker)

    dev_set = json.load(open(dataset_folder +"/dev.json"))
    dev_set = imsitu_loader_agentverbq_roles(imgset_folder, dev_set, encoder, model.dev_preprocess())
    ...
    test_set = imsitu_loader_agentverbq_roles(imgset_folder, test_set, encoder, model.dev_preprocess())
    ...
    traindev_set = imsitu_loader_agentverbq_roles(imgset_folder, traindev_set, encoder, model.dev_preprocess())
