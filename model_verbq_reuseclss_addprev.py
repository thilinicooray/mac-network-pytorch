...
class BaseModel(nn.Module):
    ...
    def forward(self, img, verb, labels):
        ...
        verb_pred_feat = self.verb_module.conv.vgg_features(img)
        verb_b4last = self.verb_module.conv.vgg_classifier[:-1](verb_pred_feat.view(-1, 512*7*7))
        ...
        if self.training:
            ...
            verb_b4last = verb_b4last.expand(3,verb_b4last.size(0), verb_b4last.size(1))
            verb_b4last = verb_b4last.transpose(0,1)
            verb_b4last = verb_b4last.contiguous().view(batch_size * 3, self.mlp_hidden*8)
            verb_pred = verb_b4last + self.dropout(verb_pred)
            verb_pred = self.verb_module.conv.vgg_classifier[-1](verb_pred)
            verb_pred = verb_pred.contiguous().view(batch_size, -1, self.n_verbs)


        else:
            verb_pred_prev = self.verb_module.conv.vgg_classifier[-1](verb_b4last)
            ...
            verb_pred = verb_b4last + self.dropout(verb_pred_logit)
            verb_pred = self.verb_module.conv.vgg_classifier[-1](verb_pred)
