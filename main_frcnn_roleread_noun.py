import torch
from imsitu_encoder_embd import imsitu_encoder
from imsitu_loader import imsitu_loader_frcnn
from imsitu_scorer_log import imsitu_scorer
import json
import mac_frcnn_roleread_noun
import os
import utils
import time
import random
...
def train(model, train_loader, dev_loader, traindev_loader, optimizer, scheduler, max_epoch, model_dir, encoder, gpu_mode, clip_norm, lr_max, model_name, args,eval_frequency=4000):
    ...
    for epoch in range(max_epoch):
        ...
        mx = len(train_loader)
        for i, (_, img, frcnn_feat, verb, roles,labels) in enumerate(train_loader):
            ...
            role_predict = pmodel(img, frcnn_feat, verb, roles)
            ...
            if total_steps % eval_frequency == 0:
                ...
                avg_score = top1_avg["value*"] + top1_avg["value-all*"]  + \
                ...
                if max_score == dev_score_list[-1]:
                    torch.save(model.state_dict(), model_dir + "/{}_macn_roleread_optupdated_frcnn_noun.model".format( model_name))
...
def eval(model, dev_loader, encoder, gpu_mode, write_to_file = True):
    ...
    with torch.no_grad():
        mx = len(dev_loader)
        for i, (img_id, img, frcnn_feat, verb, roles,labels) in enumerate(dev_loader):
            ...
            role_predict = model(img, frcnn_feat, verb, roles)
...
def main():
    ...
    parser.add_argument('--glove_file', type=str, default='./imsitu_data/glove.6B.300d.txt', help='Location of glove embeddings file')
    ...
    img_feat_dir = args.frcnn_feat_dir
    glove_file = args.glove_file

    verb_embd_file = './imsitu_data/verb_embedding.npy'
    if not os.path.exists(verb_embd_file):
        verb_embd_file = None

    role_embd_file = './imsitu_data/role_embedding.npy'
    if not os.path.exists(role_embd_file):
        role_embd_file = None


    print('model spec :, mac net v pred for training and loss calc normalizing from only matching role count + frcnn feat + word embedding')

    train_set = json.load(open(dataset_folder + "/train.json"))
    encoder = imsitu_encoder(train_set, glove_file, verb_embd_file, role_embd_file)

    model = mac_frcnn_roleread_noun.E2ENetwork(encoder, args.gpuid)

    # To group up the features
    role_features = model.parameters()
    ...
    optimizer = utils.get_optimizer_frcnn_noun(lr,weight_decay,optimizer_select,
                                            role_features)
