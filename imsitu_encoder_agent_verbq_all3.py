...
class imsitu_encoder():
        ...
        for img_id, questions in verb_questions.items():
            ...
            for question in questions:
                ...
                if "action" not in question:
                    words = words_all[:3]
                    words.append(' '.join(words_all[3:-1]))
                    words.append(words_all[-1])
    ...
    def get_qword_idx_for_agentq_top1(self, agent_set):
        agent_idx = []
        for item in agent_set:
            idx = self.label_2_qword[item]
            agent_idx.append(idx)
        return torch.tensor(agent_idx)
