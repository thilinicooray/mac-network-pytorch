...
class RecursiveGraph(nn.Module):
    ...
    def forward(self, img, img_cls, verbq, gt_verb):
        ...
        label_soft_ans, label_logits = self.role_node(img, role_qs, self.verb_lookup(gt_verb))
    ...
    def forward_eval(self, img, img_cls, verbq):
        ...
        label_soft_ans, label_logits  = self.role_node(img, role_qs, self.verb_lookup(verb))
        ...
        if args.train_all:
            ...
            for i in range(batch_size):
                for index in range(gt_labels.size()[1]):
                    ...
                    frame_loss = (verb_loss) + 0.7*(frame_loss/len(self.encoder.verb2_role_dict[self.encoder.verb_list[gt_verbs[i]]]))
