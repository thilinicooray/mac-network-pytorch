        ...
        for i, (_, img, verb, roles,labels) in enumerate(train_loader):
            ...
            if total_steps % eval_frequency == 0:
                ...
                if max_score == dev_score_list[-1]:
                    torch.save(model.state_dict(), model_dir + "/{}_mac_action.model".format( model_name))
...
def main():
    ...
    print('model spec :, mac 4 layer for actions')

    train_set = json.load(open(dataset_folder + "/train.json"))
    encoder = imsitu_encoder(train_set)

    model = model_mac_action.E2ENetwork(encoder, args.gpuid)

    # To group up the features
    cnn_features, verb_features = utils.group_features_noun(model)
    ...
    optimizer = utils.get_optimizer_verb(optimizer_select,
