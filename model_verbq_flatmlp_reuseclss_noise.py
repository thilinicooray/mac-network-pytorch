...
class BaseModel(nn.Module):
    ...
    def forward(self, img, verb, labels):
        ...
        if self.training:
            ...
            img_embd = img_embd.expand(ref_size,img_embd.size(0), img_embd.size(1), img_embd.size(2))
            img_embd = img_embd.transpose(0,1)
            img_embd = img_embd.contiguous().view(batch_size* ref_size, -1, self.mlp_hidden)

            verb_q_idx = verb_q_idx.view(batch_size*ref_size, -1)
