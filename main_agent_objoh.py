import torch
from imsitu_encoder_agent import imsitu_encoder
from imsitu_loader import imsitu_loader_agent_objoh
from imsitu_scorer_log import imsitu_scorer
import json
import model_agent_objoh
import os
import utils
...
def train(model, train_loader, dev_loader, traindev_loader, optimizer, scheduler, max_epoch, model_dir, encoder, gpu_mode, clip_norm, lr_max, model_name, args,eval_frequency=4000):
    ...
    for epoch in range(max_epoch):
        ...
        mx = len(train_loader)
        for i, (id, img, obj_oh, labels) in enumerate(train_loader):
            ...
            agent_predict = pmodel(img, obj_oh)

            '''g = make_dot(verb_predict, model.state_dict())
            g.view()'''

            loss = model.calculate_loss(agent_predict, labels)
            ...
            if total_steps % eval_frequency == 0:
                ...
                if max_score == dev_score_list[-1]:
                    torch.save(model.state_dict(), model_dir + "/{}_agentall_objoh.model".format(model_name))
...
def eval(model, dev_loader, encoder, gpu_mode, write_to_file = False):
    ...
    with torch.no_grad():
        mx = len(dev_loader)
        for i, (id, img, obj_oh, labels) in enumerate(dev_loader):
            ...
            agent_predict = model(img, obj_oh)
...
def main():
    ...
    model = model_agent_objoh.BaseModel(encoder, args.gpuid)
    ...
    train_set = imsitu_loader_agent_objoh(imgset_folder, train_set, obj_oh_set, encoder, model.train_preprocess())

    train_loader = torch.utils.data.DataLoader(train_set, batch_size=64, shuffle=True, num_workers=n_worker)

    dev_set = json.load(open(dataset_folder +"/dev.json"))
    dev_set = imsitu_loader_agent_objoh(imgset_folder, dev_set, obj_oh_set, encoder, model.dev_preprocess())
    ...
    test_set = imsitu_loader_agent_objoh(imgset_folder, test_set, obj_oh_set, encoder, model.dev_preprocess())
    ...
    traindev_set = imsitu_loader_agent_objoh(imgset_folder, traindev_set, obj_oh_set, encoder, model.dev_preprocess())
