import torch
from imsitu_encoder_agentverbq_diffeval import imsitu_encoder
from imsitu_loader import imsitu_loader_agentverbq
from imsitu_scorer_log import imsitu_scorer
import json
import model_agent2verbq_diffeval_beam
import os
import utils
...
def train(model, train_loader, dev_loader, traindev_loader, optimizer, scheduler, max_epoch, model_dir, encoder, gpu_mode, clip_norm, lr_max, model_name, args,eval_frequency=500):
    model.train()
    train_loss = 0
    total_steps = 0
    print_freq = 400
        ...
        for i, (_id, img, verb) in enumerate(train_loader):
            ...
            if total_steps % eval_frequency == 0:
                ...
                if max_score == dev_score_list[-1]:
                    torch.save(model.state_dict(), model_dir + "/{}_agent2verbq_diffeval_beam.model".format(model_name))
...
def main():
    ...
    model = model_agent2verbq_diffeval_beam.BaseModel(encoder, args.gpuid)
