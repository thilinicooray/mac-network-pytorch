...
class BaseModel(nn.Module):
    def __init__(self, encoder,
        ...
        self.role_rep_proj = nn.Linear(mlp_hidden, embed_hidden)
    ...
    def forward(self, img, verbs=None, labels=None):
        ...
        verbs = sorted_idx[:,0]
        role_pred, rep = self.role_module(img, verbs)
        ...
        #place_agent_rep = torch.index_select(rep, 1, agentplace_q_idx)
        place_agent_rep = torch.cat([ torch.index_select(a, 0, i).unsqueeze(0) for a, i in zip(rep, agentplace_q_idx) ])

        final_rep = self.role_rep_proj(place_agent_rep)
        ...
        final_q = torch.cat([final_rep, q_emb], 1)

        verb_pred_rep = self.verb_vqa(img_embd, final_q)
    ...
    def forward_eval(self, img, verbs=None, labels=None):
        ...
        verb_pred_rep = self.verb_vqa(img_embd, final_q)
        combined = verb_pred_rep_prev + verb_pred_rep
        verb_pred = self.last_class(combined)
