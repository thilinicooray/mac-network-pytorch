...
def train(model, train_loader, dev_loader, traindev_loader, optimizer, scheduler, max_epoch, model_dir, encoder, gpu_mode, clip_norm, lr_max, model_name, args,eval_frequency=4000):
    ...
    for epoch in range(max_epoch):
        ...
        for i, (img_id, img, verb, labels) in enumerate(train_loader):
            ...
            place_role_label_pred, agent_role_label_pred, other_role_label_pred = pmodel(img_id, img, verb)
            ...
            loss = model.calculate_loss(verb, place_role_label_pred, agent_role_label_pred, other_role_label_pred, labels, args)
            ...
            top1.add_point_noun_sepagentplz(verb, place_role_label_pred, agent_role_label_pred, other_role_label_pred, labels)
            top5.add_point_noun_sepagentplz(verb, place_role_label_pred, agent_role_label_pred, other_role_label_pred, labels)
            ...
            if total_steps % eval_frequency == 0:
                ...
                if max_score == dev_score_list[-1]:
                    torch.save(model.state_dict(), model_dir + "/{}_roles_independent_addverbembd_recqa_sepagentplz.model".format( model_name))
            ...
            del place_role_label_pred, agent_role_label_pred, other_role_label_pred, loss, img, verb, labels
            #break
...
def eval(model, dev_loader, encoder, gpu_mode, write_to_file = False):
    ...
    with torch.no_grad():
        ...
        for i, (img_id, img, verb, labels) in enumerate(dev_loader):
            ...
            place_role_label_pred, agent_role_label_pred, other_role_label_pred = model(img_id, img, verb)
            '''loss = model.calculate_eval_loss(verb_predict, verb, role_predict, labels)
            val_loss += loss.item()'''
            top1.add_point_noun_sepagentplz(verb, place_role_label_pred, agent_role_label_pred, other_role_label_pred, labels)
            top5.add_point_noun_sepagentplz(verb, place_role_label_pred, agent_role_label_pred, other_role_label_pred, labels)

            del place_role_label_pred, agent_role_label_pred, other_role_label_pred, img, verb, labels
            #break
...
def main():
    ...
    train_set = json.load(open(dataset_folder + "/ordered_train_new.json"))
    imsitu_roleq = json.load(open("imsitu_data/imsitu_questions_prev.json"))
    encoder = imsitu_encoder(train_set, imsitu_roleq)

    model = model_roles_recqa_sepagentplz.BaseModel(encoder, args.gpuid)
    ...
    dev_set = json.load(open(dataset_folder +"/ordered_dev_new.json"))
    ...
    test_set = json.load(open(dataset_folder +"/ordered_test_new.json"))
    ...
    traindev_set = json.load(open(dataset_folder +"/ordered_dev_new.json"))
