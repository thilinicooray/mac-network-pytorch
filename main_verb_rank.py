...
def train(model, train_loader, dev_loader, optimizer, scheduler, max_epoch, model_dir, encoder, gpu_mode, clip_norm, lr_max,eval_frequency=4000):
    model.train()
    train_loss = 0
    total_steps = 0
    print_freq = 400
    ...
    for epoch in range(max_epoch):
        ...
        for i, (p_img, p_verb, n_img, n_verb) in enumerate(train_loader):
            ...
            if total_steps % print_freq == 0:
                ...
                print('{}, {},loss = {:.2f}, avg loss = {:.2f}'.format(total_steps-1,epoch,loss.item(),
                                                                       train_loss / ((total_steps-1)%eval_frequency)))
        ...
        print('Dev {},  avg loss = {:.2f}'.format(total_steps-1, val_loss))
...
def eval(model, dev_loader, encoder, gpu_mode):
    ...
    with torch.no_grad():
        mx = len(dev_loader)
        for i, (p_img, p_verb, n_img, n_verb) in enumerate(dev_loader):
            ...
            p_img_rep, p_verb_rep = model(p_img, p_verb)
            n_img_rep, n_verb_rep = model(n_img, n_verb)
            triplet_loss = model.triplet_loss(p_img_rep, p_verb_rep, n_img_rep, n_verb_rep)
...
def main():
    ...
    model = model_verb_rank.RelationNetworks(encoder, args.gpuid)
    ...
    dev_set = imsitu_triplet_loader(imgset_folder, dev_set, encoder, args.verb_group_file, model.dev_preprocess())
    ...
    train(model, train_loader, dev_loader, optimizer, scheduler, n_epoch, model_dir, encoder, args.gpuid, clip_norm, lr_max)
