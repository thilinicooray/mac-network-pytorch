...
class BaseModel(nn.Module):
    ...
    def forward(self, image, img_id):
        ...
        agent_vis_feat = self.agent[0](conv_agent)

        agent_vis_feat = agent_vis_feat.expand(verb_q_count,agent_vis_feat.size(0), agent_vis_feat.size(1))
        agent_vis_feat = agent_vis_feat.transpose(0,1)
        agent_vis_feat = agent_vis_feat.contiguous().view(batch_size* verb_q_count, -1)

        agent_label_embd = self.w_emb(verb_qs[:,3])

        agent_label_with_feat = torch.cat([agent_label_embd, agent_vis_feat], -1)
        agent_label_with_feat = self.proj_agent(agent_label_with_feat)

        verb_q = torch.cat([self.w_emb(verb_qs[:,:3]), agent_label_with_feat.unsqueeze(1), self.w_emb(verb_qs[:,4:])], 1)
    ...
    def forward_eval(self, image, img_id):
        ...
        agent_label_embd = self.w_emb(q_word_idx)

        agent_label_with_feat = torch.cat([agent_label_embd.squeeze(), agent_vis_feat], -1)
        agent_label_with_feat = self.proj_agent(agent_label_with_feat)

        verb_q = torch.cat([self.w_emb(verbq[:,:3]), agent_label_with_feat.unsqueeze(1), self.w_emb(verbq[:,4:-1]), self.w_emb(places).unsqueeze(1)], 1)
