...
class BaseModel(nn.Module):
    ...
    def forward(self, image, verbq, topk=5):
        ...
        current_topk_agent = sorted_idx[:,0:topk]
        current_agent = sorted_idx[:,0]

        obj_weights = self.agent[-1].weight
        agent_rep = obj_weights[current_topk_agent]
        projected_obj = self.proj_obj(agent_rep)
        ...
        whole_img_feat = self.whole_img(self.conv_verb(image))
        whole_img_feat = whole_img_feat.expand(topk, whole_img_feat.size(0), whole_img_feat.size(1))
        whole_img_feat = whole_img_feat.transpose(0,1)

        combo_img = torch.cat([projected_obj, whole_img_feat], -1)

        img = self.agent4img(combo_img)
