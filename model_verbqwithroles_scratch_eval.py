...
class BaseModel(nn.Module):
    ...
    def forward(self, img_org, verb, labels):

        img_features = self.conv(img_org)
        ...
        verb_pred_prev = self.verb_module(img_org)
        sorted_idx = torch.sort(verb_pred_prev, 1, True)[1]
        verbs = sorted_idx[:,0]

        role_qs= self.encoder.get_role_questions_batch(verbs)
        if self.gpu_mode >= 0:
