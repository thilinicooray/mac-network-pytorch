...
class BaseModel(nn.Module):
    ...
    def forward(self, img, verbs=None, labels=None):
        ...
        verb_q_idx_1 = self.encoder.get_verbq_partial_idx(verbs, labels[:,0,:]).unsqueeze(1)
        verb_q_idx_2 = self.encoder.get_verbq_partial_idx(verbs, labels[:,1,:]).unsqueeze(1)
        verb_q_idx_3 = self.encoder.get_verbq_partial_idx(verbs, labels[:,2,:]).unsqueeze(1)
    ...
    def forward_eval(self, img, verbs=None, labels=None):
        ...
        verb_q_idx = self.encoder.get_verbq_partial_idx(verbs, label_idx)
