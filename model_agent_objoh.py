...
class BaseModel(nn.Module):
    def __init__(
        ...
        self.oh_to_comp = FCNet([385, mlp_hidden,mlp_hidden,mlp_hidden,mlp_hidden, mlp_hidden])
    ...
    def forward(self, image, img_obj_oh):
        ...
        #verb pred
        agent_rep = self.agent(conv)
        obj_oh = self.oh_to_comp(img_obj_oh)
        final = torch.cat([agent_rep, obj_oh], -1)
        agent_pred = self.final_layer(final)
    ...
    def calculate_loss(self, agent_pred, gt_labels):
        ...
        for i in range(batch_size):
            for index in range(gt_labels.size()[1]):
                loss += utils.cross_entropy_loss(agent_pred[i], gt_labels[i,index])
