...
class E2ENetwork(nn.Module):
    def __init__(
        ...
        self.conv_verb = vgg16_modified_verb()
        self.conv_noun = vgg16_modified()
    ...
    def forward(self, image, verbs, roles):
        conv = self.conv_verb(image)
        img_features = self.conv_noun(image)
    ...
    def forward_eval5(self, image, topk = 5):
        ...
        img_features_org = self.conv_noun(image)
        conv = self.conv_verb(image)
