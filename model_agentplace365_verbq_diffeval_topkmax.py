...
class BaseModel(nn.Module):
    ...
    def forward_eval(self, image, img_id, topk=5):
        ...
        for i in range(image.size(0)):
            self.selected_agents[img_id[i]]={'agent': sorted_idx[i][:5].tolist()}
        ...
        places = places
        places = places.expand(topk, batch_size)
        places = places.transpose(0,1)
        places = places.contiguous().view(topk * batch_size, -1)

        q_word_idx = q_word_idx.view(topk * batch_size, -1)
        verb_q = torch.cat([self.w_emb(verbq[:,:3]), self.w_emb(q_word_idx), self.w_emb(verbq[:,4:-1]), self.w_emb(places)], 1)
