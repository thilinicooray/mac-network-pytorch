import torch
from imsitu_encoder_place4verbq import imsitu_encoder
from imsitu_loader import imsitu_loader_place4verbq
from imsitu_scorer_log import imsitu_scorer
import json
import model_place4verb_alone
import os
import utils
import time
import random
...
def train(model, train_loader, dev_loader, traindev_loader, optimizer, scheduler, max_epoch, model_dir, encoder, gpu_mode, clip_norm, lr_max, model_name, args,eval_frequency=4000):
    ...
    for epoch in range(max_epoch):
        ...
        mx = len(train_loader)
        for i, (_, img, verb, verbq) in enumerate(train_loader):
            ...
            if total_steps % eval_frequency == 0:
                ...
                if max_score == dev_score_list[-1]:
                    torch.save(model.state_dict(), model_dir + "/{}_topdown_place4verb_alone.model".format( model_name))
            ...
            del verb_predict, loss, img, verb, verbq
            #break
...
def eval(model, dev_loader, encoder, gpu_mode, write_to_file = False):
    ...
    with torch.no_grad():
        mx = len(dev_loader)
        for i, (img_id, img, verb, verbq) in enumerate(dev_loader):
...
def main():
    ...
    imsitu_roleq = json.load(open("imsitu_data/places_predict_imsitu_updated_unk.json"))
    encoder = imsitu_encoder(train_set, imsitu_roleq)

    model = model_place4verb_alone.BaseModel(encoder, args.gpuid)

    # To group up the features
    cnn_features, role_features = utils.group_features_noun(model)

    train_set = imsitu_loader_place4verbq(imgset_folder, train_set, encoder, model.train_preprocess())

    train_loader = torch.utils.data.DataLoader(train_set, batch_size=64, shuffle=True, num_workers=n_worker)

    dev_set = json.load(open(dataset_folder +"/dev.json"))
    dev_set = imsitu_loader_place4verbq(imgset_folder, dev_set, encoder, model.dev_preprocess())
    ...
    test_set = imsitu_loader_place4verbq(imgset_folder, test_set, encoder, model.dev_preprocess())
    ...
    traindev_set = imsitu_loader_place4verbq(imgset_folder, traindev_set, encoder, model.dev_preprocess())
