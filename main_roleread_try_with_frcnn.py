...
def train(model, train_loader, dev_loader, traindev_loader, optimizer, scheduler, max_epoch, model_dir, encoder, gpu_mode, clip_norm, lr_max, model_name, args,eval_frequency=4000):
    ...
    for epoch in range(max_epoch):
        ...
        mx = len(train_loader)
        for i, (id, img, frcnn_feat, verb, roles,labels) in enumerate(train_loader):
            #print('current batch :', id)
            ...
            verb_predict, role_predict = pmodel(img, frcnn_feat, verb, roles)
            ...
            if total_steps % eval_frequency == 0:
                ...
                if max_score == dev_score_list[-1]:
                    torch.save(model.state_dict(), model_dir + "/{}_mac_roleread_testing_with_linearinit.model".format( model_name))
            ...
            del verb_predict, role_predict, loss, img, frcnn_feat, verb, roles, labels
            #break
...
def eval(model, dev_loader, encoder, gpu_mode, write_to_file = True):
    ...
    with torch.no_grad():
        ...
        for i, (img_id, img, frcnn_feat, verb, roles,labels) in enumerate(dev_loader):
            ...
            verb_predict, _, role_predict = model.forward_eval5(img, frcnn_feat)
            ...
            del verb_predict, role_predict, img, frcnn_feat, verb, roles, labels
            #break
...
def main():
    ...
    print('model spec :, mac 4 layer roleread with both grid and frcnn img')

    train_set = json.load(open(dataset_folder + "/train.json"))
    encoder = imsitu_encoder(train_set)

    model = roleread_mac_testing.E2ENetwork(encoder, args.gpuid)
