...
class TopDown(nn.Module):
    ...
    def forward(self, img, q):
        ...
        return v_repr, q_repr, joint_repr
...
class BaseModel(nn.Module):
    ...
    def forward(self, img_id, img, verb):
        ...
        v_rep, q_rep_first, rep = self.roles(img_updated, q_emb)

        for i in range(self.num_steps):
            labelrep =  v_rep * q_rep_first
            labelrep = labelrep.contiguous().view(batch_size, -1, self.mlp_hidden)
            ...
            v_rep, _, rep2 = self.roles(img_updated, q_emb_up)
