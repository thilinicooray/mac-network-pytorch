...
class TopDown(nn.Module):
    ...
    def forward(self, img_org, q):
        ...
        img_org_exp = img_org.expand(self.max_role_count -1 ,img_org.size(0), img_org.size(1), img_org.size(2))
        img_org_exp = img_org_exp.transpose(0,1)
        img_org_exp = img_org_exp.contiguous().view(img_org.size(0)* (self.max_role_count-1), -1, self.mlp_hidden)
        ...
        for i in range(3):
            ...
            v_emb = (att * ctx_img).sum(1) # [batch, v_dim]
