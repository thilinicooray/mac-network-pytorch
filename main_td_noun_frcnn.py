...
from imsitu_loader import imsitu_loader_frcnn_roleq
from imsitu_scorer_log import imsitu_scorer
import json
import model_top_down_frcnn
import os
import utils
import time
import random
...
def train(model, train_loader, dev_loader, traindev_loader, optimizer, scheduler, max_epoch, model_dir, encoder, gpu_mode, clip_norm, lr_max, model_name, args,eval_frequency=4000):
    ...
    for epoch in range(max_epoch):
        ...
        mx = len(train_loader)
        for i, (_, img, frcnn_feat, verb, roles,role_q, q_len, labels) in enumerate(train_loader):
            ...
            role_predict = pmodel(frcnn_feat, verb, role_q)
...
def eval(model, dev_loader, encoder, gpu_mode, write_to_file = False):
    ...
    with torch.no_grad():
        mx = len(dev_loader)
        for i, (img_id, img, frcnn_feat, verb, roles,role_q, q_len, labels) in enumerate(dev_loader):
...
def main():
    ...
    model = model_top_down_frcnn.BaseModel(encoder, args.gpuid)
    ...
    traindev_set = imsitu_loader_frcnn_roleq(imgset_folder, img_feat_dir, traindev_set, encoder, model.dev_preprocess())
