...
from imsitu_loader import imsitu_loader_rotation, _collate_fun
from imsitu_scorer_log import imsitu_scorer
import json
import model_self_supervised_vgg
import os
import utils
import time
import random
...
def train(model, train_loader, dev_loader, traindev_loader, optimizer, scheduler, max_epoch, model_dir, encoder, gpu_mode, clip_norm, lr_max, model_name, args,eval_frequency=4000):
    ...
    for epoch in range(max_epoch):
        ...
        for i, (img, rot) in enumerate(train_loader):
            ...
            loss = model.calculate_loss(rot_predict, rot)
            ...
            if total_steps % print_freq == 0:
                top1_a = top1.get_average_rot_results()
            ...
            if total_steps % eval_frequency == 0:
                ...
                top1_avg = top1.get_average_rot_results()

                avg_score = top1_avg["rot"]
...
def main():
    ...
    model = model_self_supervised_vgg.BaseModel(encoder, args.gpuid)
    ...
    train_loader = torch.utils.data.DataLoader(train_set, batch_size=16, shuffle=True, num_workers=n_worker, collate_fn=_collate_fun)

    dev_set = json.load(open(dataset_folder +"/dev.json"))
    dev_set = imsitu_loader_rotation(imgset_folder, dev_set, encoder, model.dev_preprocess())
    dev_loader = torch.utils.data.DataLoader(dev_set, batch_size=16, shuffle=True, num_workers=n_worker, collate_fn=_collate_fun)
    ...
    traindev_set = imsitu_loader_rotation(imgset_folder, traindev_set, encoder, model.dev_preprocess())
    ...
    optimizer = torch.optim.Adamax([{'params': cnn_features, 'lr': 1e-2},
