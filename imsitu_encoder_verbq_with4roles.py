...
class imsitu_encoder():
    def __init__(self, train_set, role_questions, verb_templates):
        ...
        self.vrole_question = {}
        self.verb_q_templates = verb_templates
        self.verb_q_words = {}
        self.max_verbq_count = 0
        ...
        self.other_item_roles= [ 'rammingitem', 'wrappeditem', 'wrappingitem', 'goalitem', 'paweditem', 'heaveditem', 'destroyeditem',
                     'item1', 'sliceditem', 'signeditem', 'caughtitem', 'items', 'turneditem', 'scrapeditem', 'createditem',
                     'drencheditem', 'deflecteditem', 'releaseditem', 'harvesteditem', 'removeditem', 'hidingitem', 'placeditem',
                     'crusheditem']
        self.other_tool_roles = [ 'firearm', 'instrument']
        ...
        #add verb question templates and fillers
        for l_id in range(len(self.label_list)):
            self.verb_q_words[self.label_list[l_id]] = l_id

        for verb, details in self.verb_q_templates.items():
            words = details['template'].split()
            if len(words) > self.max_verbq_count:
                self.max_verbq_count = len(words)

            for word in words:
                if word not in self.verb_q_words:
                    self.verb_q_words[word] = len(self.verb_q_words)
        ...
        print('train set stats: \n\t verb count:', len(self.verb_list), '\n\t role count:',len(self.role_list),
              ...
              '\n\t max verb q word count:', self.max_verbq_count)
    ...
    def get_verbq_idx(self, verb_ids, label_ids):
        ...
        for i in range(batch_size):
            ...
            if verb_details['has_tool']:
                if 'tool' in current_role_list:
                    tool_idx = current_role_list.index('tool')
                else:
                    for a_role in self.other_tool_roles:
                        if a_role in current_role_list:
                            tool_idx = current_role_list.index(a_role)
                            break
                tool_name = self.label_list[current_labels[tool_idx]]
            if verb_details['has_item']:
                if 'item' in current_role_list:
                    item_idx = current_role_list.index('item')
                else:
                    for a_role in self.other_item_roles:
                        if a_role in current_role_list:
                            item_idx = current_role_list.index(a_role)
                            break
                item_name = self.label_list[current_labels[item_idx]]


            if len(agent_name) > 0:
                q_part_agent = ' the ' + agent_name + ' doing'
            else:
                q_part_agent = ''
            if len(place_name) > 0:
                q_part_place = ' at the ' + place_name
            else:
                q_part_place = ''
            if len(item_name) > 0:
                q_part_item = ' with the ' + item_name
            else:
                q_part_item = ''
            if len(tool_name) > 0:
                q_part_tool = ' using the ' + tool_name
            else:
                q_part_tool = ''

            if len(q_part_agent) > 0:
                question = 'what is' + q_part_agent + q_part_item + q_part_tool + q_part_place
            else:
                question = 'what is happening' + q_part_place
            ...
            for word in words:
                q_idx.append(self.verb_q_words[word])
            padding_words = self.max_verbq_count - len(q_idx)
