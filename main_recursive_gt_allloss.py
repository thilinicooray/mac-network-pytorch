...
import json
import model_recursive_gt_allloss
import os
import utils
import time
import random
...
def train(model, train_loader, dev_loader, traindev_loader, optimizer, scheduler, max_epoch, model_dir, encoder, gpu_mode, clip_norm, lr_max, model_name, args,eval_frequency=4000):
    ...
    for epoch in range(max_epoch):
        ...
        for i, (_, img, verb, verbq, ans) in enumerate(train_loader):
            ...
            loss_res, verb_predict, label_predict = pmodel(img, verbq, verb)
            ...
            loss = model.calculate_loss(loss_res[0], verb, loss_res[1], ans, args)
            ...
            if total_steps % eval_frequency == 0:
                ...
                if max_score == dev_score_list[-1]:
                    torch.save(model.state_dict(), model_dir + "/{}_recursive_gtallloss.model".format( model_name))
...
def eval(model, dev_loader, encoder, gpu_mode, write_to_file = False):
    ...
    with torch.no_grad():
        ...
        for i, (img_id, img, verb, verbq, ans) in enumerate(dev_loader):
            ...
            loss_res, verb_predict, label_predict = model(img, verbq, None)
...
def main():
    ...
    model = model_recursive_gt_allloss.BaseModel(encoder, args.gpuid)
