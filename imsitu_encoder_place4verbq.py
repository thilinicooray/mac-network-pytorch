...
class imsitu_encoder():
    def __init__(self, train_set, places):
        ...
        self.verb_question = {}
        self.verbq_phrase = 'what is the action happening at the '
        self.places = places

        for img_id, place in places.items():
            question = self.verbq_phrase + place
            self.verb_question[img_id] = question
            words = nltk.word_tokenize(self.verbq_phrase)
    ...
    def get_verbq(self, id):
        vquestion_tokens = []
        question = self.verbq_phrase

        words = nltk.word_tokenize(question)
        words = words[:-1]
        for word in words:
            vquestion_tokens.append(self.question_words[word])
        vquestion_tokens.append(self.question_words[self.places[id]])
