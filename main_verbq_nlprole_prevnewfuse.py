import torch
from imsitu_encoder_roleq_verbtemplate_n_commonq import imsitu_encoder
from imsitu_loader import imsitu_loader_roleq_updated
from imsitu_scorer_log import imsitu_scorer
import json
import model_verbq_nlpqrole_prevnewfuse
import os
import utils
import time
import random
...
def train(model, train_loader, dev_loader, traindev_loader, optimizer, scheduler, max_epoch, model_dir, encoder, gpu_mode, clip_norm, lr_max, model_name, args,eval_frequency=4000):
    ...
    for epoch in range(max_epoch):
        ...
        for i, (id, img, verb, labels) in enumerate(train_loader):
            ...
            '''print('grad check cnn:')
            for f in model.conv.parameters():
                print('data is')
                print(f.data[0][0])
                break

            print('grad check lstm:')
            for f in model.verb_vqa.q_emb.parameters():
                print('data is')
                print(f.data[0][0])
                break

            print('grad check class:')
            for f in model.verb_vqa.classifier.parameters():
                print('data is')
                print(f.data[0][0])
                break

            print('grad check last:')
            for f in model.last_class.parameters():
                print('data is')
                print(f.data[0][0])
                break'''
            ...
            if total_steps % eval_frequency == 0:
                ...
                if max_score == dev_score_list[-1]:
                    torch.save(model.state_dict(), model_dir + "/{}_verbq_final_deprole_verbqa0_prevnewfuse_cls.model".format( model_name))
...
def main():
    ...
    model = model_verbq_nlpqrole_prevnewfuse.BaseModel(encoder, args.gpuid)
    ...
    if args.resume_training:
        ...
        utils.load_net_nlpq_fuse_ft(args.resume_model, [model])
        optimizer_select = 0
    ...
    utils.set_trainable(model.verb_vqa.classifier, True)
    utils.set_trainable(model.last_class, True)
