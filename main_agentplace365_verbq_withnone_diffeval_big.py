...
import json
import model_agentplace365_verbq_withnone_diffeval_big
import os
import utils
import time
import random
        ...
        for i, (img_id, img, verb) in enumerate(train_loader):
            ...
            if total_steps % eval_frequency == 0:
                ...
                if max_score == dev_score_list[-1]:
                    torch.save(model.state_dict(), model_dir + "/{}_agentplace365_verbq_td_cnndirectagpred_withnone_plzpred_big.model".format( model_name))
...
def main():
    ...
    model = model_agentplace365_verbq_withnone_diffeval_big.BaseModel(encoder, args.gpuid)

    # To group up the features
    cnn_agent_features, cnn_verb_features, verb_features = utils.group_features_agent2verb_directcnn(model)
    ...
    optimizer = utils.get_optimizer_agent2verb_directagentcnn(lr,weight_decay,optimizer_select,
