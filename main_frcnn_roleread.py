        ...
        for i, (_, img, frcnn_feat, verb, roles,labels) in enumerate(train_loader):
            ...
            if total_steps % eval_frequency == 0:
                ...
                if max_score == dev_score_list[-1]:
                    torch.save(model.state_dict(), model_dir + "/{}_macn_roleread_optupdated_frcnn.model".format( model_name))
...
def main():
    ...
    print('model spec :, mac net v pred for training and loss calc normalizing from only matching role count + frcnn feat ')

    train_set = json.load(open(dataset_folder + "/train.json"))
    encoder = imsitu_encoder(train_set)

    model = mac_frcnnin_roleread.E2ENetwork(encoder, args.gpuid)
    ...
    train_loader = torch.utils.data.DataLoader(train_set, batch_size=56, shuffle=True, num_workers=n_worker)
