...
def train(model, train_loader, dev_loader, traindev_loader, optimizer, scheduler, max_epoch, model_dir, encoder, gpu_mode, clip_norm, lr_max, model_name, args,eval_frequency=4000):
    ...
    for f in model.verb_q_emb.parameters():
    ...
    for epoch in range(max_epoch):
        ...
        for i, (id, img, verb, labels) in enumerate(train_loader):
            ...
            for f in model.real_comb_concat.parameters():
                print('data is')
                print(f.data [0][0])
                #print('grad is')
            ...
            if total_steps % eval_frequency == 0:
                ...
                if max_score == dev_score_list[-1]:
                    torch.save(model.state_dict(), model_dir + "/{}_verbq_roleattusing.model".format( model_name))
...
def main():
    ...
    verbq = json.load(open(dataset_folder + "/verb_questions_updated.json"))
    imsitu_roleq = json.load(open("imsitu_data/imsitu_questions_prev.json"))
    encoder = imsitu_encoder(train_set, verbq, imsitu_roleq)

    model = model_verbq_basedonroles.BaseModel(encoder, args.gpuid)

    # To group up the features
    cnn_features, role_features, other_features = utils.group_features_verbqiter(model)
