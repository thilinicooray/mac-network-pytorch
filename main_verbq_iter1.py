...
def train(model, train_loader, dev_loader, traindev_loader, optimizer, scheduler, max_epoch, model_dir, encoder, gpu_mode, clip_norm, lr_max, model_name, args,eval_frequency=64):
    model.train()
    train_loss = 0
    total_steps = 0
    print_freq = 400
        ...
        for i, (id, img, verb, labels) in enumerate(train_loader):
            ...
            if total_steps % eval_frequency == 0:
                ...
                if max_score == dev_score_list[-1]:
                    torch.save(model.state_dict(), model_dir + "/{}_verbq_iter1.model".format( model_name))
...
def main():
    ...
    model = model_verbq_iter1.BaseModel(encoder, args.gpuid)
