...
class vgg16_modified_agent(nn.Module):
    def __init__(self, num_classes):
        super(vgg16_modified_agent, self).__init__()
...
class BaseModel(nn.Module):
    def __init__(
        ...
        self.conv_agent = vgg16_modified_agent(self.vocab_size)
    ...
    def forward_eval(self, image, img_id):
        ...
        agent_logit = self.conv_agent(image)
        ...
        q_word_idx = self.encoder.get_qword_idx_for_agentq_top1(current_agent, img_id)
        if self.gpu_mode >= 0:
            q_word_idx = q_word_idx.to(torch.device('cuda'))

        verb_q = self.w_emb(q_word_idx)
