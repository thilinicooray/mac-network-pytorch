...
import json
import model_verbq_iter_embdhz_sepiter
import os
import utils
import time
import random
...
def train(model, train_loader, dev_loader, traindev_loader, optimizer, scheduler, max_epoch, model_dir, encoder, gpu_mode, clip_norm, lr_max, model_name, args,eval_frequency=4000):
    ...
    for epoch in range(max_epoch):
        ...
        for i, (id, img, verb, labels) in enumerate(train_loader):
            ...
            verb_predict_rep0, verb_pred0 = pmodel.forward0(img, verb, labels)
            loss1 = model.calculate_loss_single(verb_predict_rep0, verb)

            verb_predict = pmodel.forward1(img, verb_predict_rep0, verb, labels)
            ...
            '''if gpu_mode >= 0 :
                loss.backward(torch.ones([4,1]).to(torch.device('cuda')))
            else:
                loss.backward()'''
            loss.backward()
            ...
            if total_steps % eval_frequency == 0:
                ...
                if max_score == dev_score_list[-1]:
                    torch.save(model.state_dict(), model_dir + "/{}_verbq_iter_embdhz.model".format( model_name))
...
def main():
    ...
    model = model_verbq_iter_embdhz_sepiter.BaseModel(encoder, args.gpuid)
