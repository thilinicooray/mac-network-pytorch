...
class TopDown(nn.Module):
    def __init__(self,
        ...
        self.flat = nn.Linear(mlp_hidden * 7 *7 + mlp_hidden, mlp_hidden*8)
...
class BaseModel(nn.Module):
    ...
    def forward(self, img, verb, labels):
        ...
        verb_pred_rep = self.verb_vqa(img_embd, qw_emb)
        verb_pred_prev = self.classifier(verb_pred_rep)
        ...
        verb_pred_rep_i1 = self.verb_vqa(combo, qw_emb)

        rep = verb_pred_rep + self.dropout(verb_pred_rep_i1)

        verb_pred_i1 = self.classifier(rep)
