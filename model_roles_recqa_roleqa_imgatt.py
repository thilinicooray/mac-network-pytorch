...
class BaseModel(nn.Module):
    def __init__(self, encoder,
        ...
        self.role_combine = nn.Linear(mlp_hidden, mlp_hidden)
    ...
    def forward(self, img, verb):
        ...
        rep = self.roles(img_updated_org, q_emb)
        ...
        for i in range(self.num_steps - 1):
            ...
            img_updated_new = img_updated_new.contiguous().view(img_updated_org.size(0)* (self.max_role_count - 1), -1, self.mlp_hidden)


            neighbours = self.role_combine(labelrep_expand).unsqueeze(1)
            rolewise = neighbours * img_updated_new
            added_all = torch.sum(rolewise.view(-1,self.max_role_count - 1, rolewise.size(1), rolewise.size(2) ), 1)
